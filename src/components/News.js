    ...
    if (!isLoaded) {
      ...
      const NewsItem = (article) => (
        <Card key={article.url}>
          ...
          <Card.Body>
            <p className="card-text"><small className="text-muted">{article.publishedAt.slice(0, 10)}</small></p>
            <a href={article.url}>          
            ...
            <footer className="blockquote-footer">
              By {article.author} for <cite title="Source Author">{article.source.name}</cite>
            </footer>
          </Card.Body>
        </Card>
      );
